...
## Aktualizacja techniczna 2026-03-31 — implementacja opcji A2 + B2

### 1) Dwa manifesty (user/admin)
Dodano dwa pliki manifestu na poziomie repozytorium:
- `manifest.webmanifest` — wariant user:
  - `start_url: ./Main/index.html`
- `manifest-admin.webmanifest` — wariant admin:
  - `start_url: ./Main/index.html?admin=1`

Oba manifesty mają wspólne parametry PWA:
- `scope: ./`
- `display: standalone`
- `background_color: #000000`
- `theme_color: #031605`

### 2) Dynamiczny wybór manifestu w `Main/index.html`
W `<head>` link do manifestu otrzymał identyfikator:
- `<link rel="manifest" id="appManifest" href="../manifest.webmanifest">`

Bezpośrednio po nim dodano skrypt IIFE `setManifestByMode()`:
1. Odczytuje `window.location.search` przez `URLSearchParams`.
2. Jeżeli `admin=1`, ustawia `href` linku manifestu na `../manifest-admin.webmanifest`.
3. W przeciwnym razie pozostawia/ustawia `../manifest.webmanifest`.

Efekt:
- Skrót/PWA utworzone w trybie user ma punkt startowy user.
- Skrót/PWA utworzone w trybie admin może użyć punktu startowego admin (`?admin=1`).

### 3) Opcja B2 (tymczasowa): usunięcie `maskable`
W obu manifestach usunięto deklarację `purpose: "any maskable"`.
Aktualnie wszystkie ikony używają wyłącznie:
- `purpose: "any"`.

Dotyczy to wpisów:
- `IkonaGlowna.png` 192x192,
- `IkonaGlowna.png` 512x512,
- `IkonaPowiadomien.png` 192x192.

### 4) Service Worker i cache manifestów
W `service-worker.js`:
- podniesiono `SW_VERSION` do `wg-pwa-v4`,
- do `APP_SHELL_ASSETS` dodano `./manifest-admin.webmanifest`.

Dzięki temu offline cache obejmuje oba warianty manifestu i po aktywacji nowej wersji SW stare cache są czyszczone zgodnie z istniejącą logiką `activate`.
