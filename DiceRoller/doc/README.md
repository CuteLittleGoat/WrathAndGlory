# DiceRoller

## Instrukcja obsługi (PL)
1. Otwórz `DiceRoller/index.html` w przeglądarce.
2. Ustaw **Stopień Trudności** (1-99) — wymagana liczba punktów do sukcesu.
3. Ustaw **Pulę Kości** (1-99) — liczbę kości do rzutu.
4. Ustaw **Ilość Kości Furii** (1-99) — liczba czerwonych kości; nie może być większa niż Pula Kości.
5. Kliknij **Rzuć kośćmi!**
6. W trakcie rzutu kości wykonają trójwymiarową animację i odbiją się wewnątrz żółtej ramki.
7. Po animacji zobaczysz:
   - wynik każdej kości,
   - łączną liczbę punktów,
   - informację **Sukces!** lub **Porażka!**,
   - ewentualny komunikat **Komplikacja Furii 🙁** (jeśli na czerwonej kości wypadła 1),
   - ewentualny komunikat **Krytyczna Furia 🙂** (jeśli wszystkie czerwone kości wyrzuciły 6),
   - informację **Możliwe Przeniesienie: X** (liczba nadmiarowych szóstek do przeniesienia).

## User guide (EN)
1. Open `DiceRoller/index.html` in your browser.
2. Set the **Difficulty** (1-99) — required points for success.
3. Set the **Dice Pool** (1-99) — number of dice to roll.
4. Set the **Fury Dice** (1-99) — number of red dice; it cannot exceed the Dice Pool.
5. Click **Rzuć kośćmi!**
6. During the roll the dice perform a 3D animation and bounce inside the yellow frame.
7. After the animation you will see:
   - each die result,
   - total points,
   - **Sukces!** or **Porażka!** message,
   - **Komplikacja Furii 🙁** (if any red die rolled 1),
   - **Krytyczna Furia 🙂** (if all red dice rolled 6),
   - **Możliwe Przeniesienie: X** (how many extra sixes can be carried over).
