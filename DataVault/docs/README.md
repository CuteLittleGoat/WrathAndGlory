...
- **Tryb gracza (domyślny)**
  ...
  - Ukrywa zakładkę **Bestiariusz** (dostępną tylko dla admina).
- **Tryb admina**
...
### Overview
A static web application for browsing Wrath & Glory data in tables. It runs in the browser with no backend and no local dependencies — you only need to host the static files. The UI uses the green theme and font stack aligned with `Main/index.html`, with secondary text highlights (`#4FAF4F`) and brighter reference text (`#D2FAD2`). The **Tabela Rozmiarów** tab always sits between **Bestiariusz** and **Gatunki**, while **Gatunki** sits between **Tabela Rozmiarów** and **Archetypy**; **Bonusy Frakcji** sits between **Archetypy** and **Słowa Kluczowe Frakcji**, and **Słowa Kluczowe Frakcji** sits between **Bonusy Frakcji** and **Implanty Astartes**; **Zakony Pierwszego Powołania** sit between **Implanty Astartes** and **Ścieżki Asuryani**, **Orcze Klany** sit between **Ścieżki Asuryani** and **Mutacje Krootów**, and **Mutacje Krootów** sit between **Orcze Klany** and **Cechy**. The **Tabela Rozmiarów** tab includes columns ordered as **Rozmiar** (8ch), **Modyfikator Testu Ataku** (26ch), **Zmniejszenie Poziomu Ukrycia** (26ch), and **Przykłady** (85ch), with the modifier columns centered and the **Przykłady** column left-aligned. The **Archetypy** tab includes columns for PD cost, keywords, archetype attributes, skills, abilities, equipment, and extra notes; **Atrybuty Archetypu** and **Umiejętności Archetypu** are 26ch wide, while **Zdolność Archetypu** and **Ekwipunek** are 46ch each. **Bonusy Frakcji** uses **Frakcja** at 26ch and **Premia 1–3** at 56ch each; **Słowa Kluczowe Frakcji** uses **Frakcja** 26ch, **Słowo Kluczowe** 28ch, **Efekt** 56ch, and **Opis** 26ch; **Implanty Astartes** uses **Numer** 4ch, **Nazwa** 26ch, **Opis** 26ch; **Zakony Pierwszego Powołania** uses **Nazwa** 26ch, **Opis** 56ch, **Zaleta** 26ch, **Wada** 26ch; **Ścieżki Asuryani** and **Orcze Klany** use **Nazwa** 26ch, **Efekt** 26ch, **Opis** 56ch; **Mutacje Krootów** uses **Mutacja Krootów** 26ch, **Pożarta Ofiara** 26ch, **Efekt** 26ch, **Opis** 56ch. Numeric columns (Bestiary stats, costs, availability, weapon parameters, ST, and ranges) are center-aligned; the `Zasięg` column in **Bronie** does not wrap. In **Słowa Kluczowe Frakcji**, the **Słowo Kluczowe** column is red except for the `-` token and the word `lub`, preserves italic styling coming from the sheet (e.g., the italic `lub`), and treats `[ŚWIAT-KUŹNIA]` as fully red. The “Czy wyświetlić zakładki dotyczące tworzenia postaci?” checkbox (unchecked by default) shows the **Tabela Rozmiarów**, **Gatunki**, **Archetypy**, **Bonusy Frakcji**, **Słowa Kluczowe Frakcji**, **Implanty Astartes**, **Zakony Pierwszego Powołania**, **Ścieżki Asuryani**, **Orcze Klany**, and **Mutacje Krootów** tabs (when unchecked, those tabs stay hidden). Tabs tied to this checkbox use the lighter `#D2FAD2` text color to match the checkbox label.
