...
- PL: W oknie logowania wyświetla się ikona `IkonaPowiadomien2.png` w stałym polu (72×72 px), więc karta logowania nie zmienia rozmiaru podczas doczytywania zasobów.
- EN: The login window now shows `IkonaPowiadomien2.png` in a fixed 72×72 px slot, so the login card keeps a stable size while assets load.
...
### Aktualna uwaga o generatorze referencyjnym (PL)
...
### Current note about the reference generator (EN)
