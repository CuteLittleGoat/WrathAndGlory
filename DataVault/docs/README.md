...
### Opis
Statyczna aplikacja webowa do przeglądania danych z systemu Wrath & Glory w formie tabel. Działa w przeglądarce, nie wymaga backendu ani instalacji zależności — wystarczy hostować pliki statyczne. Interfejs korzysta z zielonego motywu i fontu zgodnego z `Main/index.html`, z wyróżnieniem tekstu pomocniczego (`#4FAF4F`) i jaśniejszego tekstu referencji (`#D2FAD2`). Zakładka **Tabela_Rozmiarow** znajduje się zawsze między **Bestiariusz** a **Archetypy**; zakładka **Bonusy_Frakcji** znajduje się między **Archetypy** a **Cechy** i ma kolumny **Frakcja** (26ch) oraz **Premia 1–3** (po 56ch). Zakładka **Tabela_Rozmiarow** (tylko admin) zawiera kolumny w kolejności **Rozmiar** (8ch), **Modyfikator Testu Ataku** (46ch), **Zmniejszenie Poziomu Ukrycia** (46ch) oraz **Przykłady** (82ch), z wyrównaniem wartości modyfikatorów do środka. Zakładka **Archetypy** zawiera m.in. kolumny kosztu PD, słów kluczowych, atrybutów, umiejętności, zdolności, ekwipunku i dodatkowych informacji; kolumny **Atrybuty Archetypu** i **Umiejętności Archetypu** mają po 28ch, a **Zdolność Archetypu** i **Ekwipunek** po 46ch. Kolumny liczbowe (statystyki Bestiariusza, koszty, dostępność, parametry broni, ST i zasięgi) są wyrównane do środka; w **Broniach** kolumna `Zasięg` nie łamie wierszy.
