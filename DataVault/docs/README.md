...
W tej wersji repozytorium `data.json` został ponownie wygenerowany na podstawie aktualnego `Repozytorium.xlsx`, więc tabele w aplikacji odpowiadają najnowszemu arkuszowi.
...
In this repository snapshot, `data.json` has been regenerated from the latest `Repozytorium.xlsx`, so the tables reflect the newest spreadsheet content.
