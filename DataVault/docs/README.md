...
### Opis
Statyczna aplikacja webowa do przeglądania danych z systemu Wrath & Glory w formie tabel. Działa w przeglądarce, nie wymaga backendu ani instalacji zależności — wystarczy hostować pliki statyczne. Interfejs korzysta z zielonego motywu i fontu zgodnego z `Main/index.html`, z wyróżnieniem tekstu pomocniczego (`#4FAF4F`) i jaśniejszego tekstu referencji (`#D2FAD2`). Zakładka **Tabela_Rozmiarow** znajduje się zawsze między **Bestiariusz** a **Gatunki**, a **Gatunki** są między **Tabela_Rozmiarow** a **Archetypy**; zakładka **Bonusy_Frakcji** znajduje się między **Archetypy** a **Slowa_Kluczowe_Frakcji**, a **Slowa_Kluczowe_Frakcji** między **Bonusy_Frakcji** a **Implanty_Astartes**; **Sciezki_Asuryani** są między **Implanty_Astartes** a **Mutacje_Krootow**, a **Mutacje_Krootow** między **Sciezki_Asuryani** a **Cechy**. Zakładka **Tabela_Rozmiarow** (tylko admin) zawiera kolumny w kolejności **Rozmiar** (8ch), **Modyfikator Testu Ataku** (25ch), **Zmniejszenie Poziomu Ukrycia** (25ch) oraz **Przykłady** (85ch), z wyrównaniem wartości modyfikatorów do środka i lewym wyrównaniem kolumny **Przykłady**. Zakładka **Archetypy** zawiera m.in. kolumny kosztu PD, słów kluczowych, atrybutów, umiejętności, zdolności, ekwipunku i dodatkowych informacji; kolumny **Atrybuty Archetypu** i **Umiejętności Archetypu** mają po 28ch, a **Zdolność Archetypu** i **Ekwipunek** po 46ch. Kolumny liczbowe (statystyki Bestiariusza, koszty, dostępność, parametry broni, ST i zasięgi) są wyrównane do środka; w **Broniach** kolumna `Zasięg` nie łamie wierszy. W zakładce **Slowa_Kluczowe_Frakcji** kolumna **Słowo Kluczowe** jest czerwona poza tokenami `-` i `lub`. Checkbox „Czy wyświetlić zakładki dotyczące tworzenia postaci?” (domyślnie odznaczony) pozwala ukryć zakładki: **Tabela_Rozmiarow**, **Archetypy**, **Bonusy_Frakcji**, **Gatunki**, **Slowa_Kluczowe_Frakcji**, **Sciezki_Asuryani**, **Mutacje_Krootow**.
...
### Overview
A static web application for browsing Wrath & Glory data in tables. It runs in the browser with no backend and no local dependencies — you only need to host the static files. The UI uses the green theme and font stack aligned with `Main/index.html`, with secondary text highlights (`#4FAF4F`) and brighter reference text (`#D2FAD2`). The **Tabela_Rozmiarow** tab always sits between **Bestiariusz** and **Gatunki**, while **Gatunki** sits between **Tabela_Rozmiarow** and **Archetypy**; **Bonusy_Frakcji** sits between **Archetypy** and **Slowa_Kluczowe_Frakcji**, and **Slowa_Kluczowe_Frakcji** sits between **Bonusy_Frakcji** and **Implanty_Astartes**; **Sciezki_Asuryani** sit between **Implanty_Astartes** and **Mutacje_Krootow**, while **Mutacje_Krootow** sit between **Sciezki_Asuryani** and **Cechy**. The **Tabela_Rozmiarow** tab (admin only) includes columns ordered as **Rozmiar** (8ch), **Modyfikator Testu Ataku** (25ch), **Zmniejszenie Poziomu Ukrycia** (25ch), and **Przykłady** (85ch), with the modifier columns centered and the **Przykłady** column left-aligned. The **Archetypy** tab includes columns for PD cost, keywords, archetype attributes, skills, abilities, equipment, and extra notes; **Atrybuty Archetypu** and **Umiejętności Archetypu** are 28ch wide, while **Zdolność Archetypu** and **Ekwipunek** are 46ch each. Numeric columns (Bestiary stats, costs, availability, weapon parameters, ST, and ranges) are center-aligned; the `Zasięg` column in **Bronie** does not wrap. In **Slowa_Kluczowe_Frakcji**, the **Słowo Kluczowe** column is red except for the `-` token and the word `lub`. The “Czy wyświetlić zakładki dotyczące tworzenia postaci?” checkbox (unchecked by default) hides the **Tabela_Rozmiarow**, **Archetypy**, **Bonusy_Frakcji**, **Gatunki**, **Slowa_Kluczowe_Frakcji**, **Sciezki_Asuryani**, and **Mutacje_Krootow** tabs.
