...
4. Pod tabelami **Atrybuty** i **Umiejętności** znajduje się tabela referencyjna maksymalnych wartości atrybutów dla ras (PL/EN), z wyśrodkowanymi wartościami i naprzemiennym tłem wierszy (zebra striping).
5. Pod tabelą referencyjną widoczna jest stała informacja o limicie umiejętności: `8`.
6. Przycisk **Resetuj wartości** ustawia wszystkie pola na `0`.
...
- W `KalkulatorXP.html` dodano pod tabelami **Atrybuty** i **Umiejętności** nową, statyczną tabelę referencyjną maksymalnych wartości atrybutów (10 ras × 8 atrybutów), z danymi zaszytymi w kodzie i tłumaczeniami PL/EN.
- Wartości w tabeli referencyjnej są wyśrodkowane i mają naprzemienne tło wierszy (zebra striping), zgodne wizualnie z motywem aplikacji.
- Dodano linię informacyjną o stałym limicie umiejętności (`8`) pod tabelą referencyjną.
...
4. Below the **Attributes** and **Skills** tables there is a reference table with maximum attribute values by species (PL/EN), with centered values and zebra striping.
5. Under the reference table, a constant skill limit note is displayed: `8`.
6. The **Reset values** button sets all fields to `0`.
...
- Added a new static reference table below **Attributes** and **Skills** in `KalkulatorXP.html` with maximum attribute values (10 species × 8 attributes), with data embedded directly in code and full PL/EN labels.
- Reference-table values are center-aligned and use zebra striping consistent with the app visual style.
- Added a fixed skill limit note (`8`) under the reference table.
